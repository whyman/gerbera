...
function upload_to_bintray() {
  ...
  curl \
    ...
    -H "X-Bintray-Debian-Distribution: $ubuntu_codename" \
...
if [[ "$ubuntu_codename" == "bionic" ]]; then
  # cmake pulls g++ which changes your GCC symlinks because ubuntu knows better than you
...
deb_version="${git_ver}-${ubuntu_codename}1"
is_tag=1
if [[ $git_ver == *"+"* ]]; then
  deb_version="${git_ver}~${commit_date}-${ubuntu_codename}1"
  is_tag=0
fi
